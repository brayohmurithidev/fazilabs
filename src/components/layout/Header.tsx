...
export default function Header() {
  ...
  return (
    <>
        ...
        <nav className="max-w-[90vw] mx-auto flex justify-between items-center px-4 md:px-8 py-3 md:py-4">
          ...
          <Link to="/" aria-label="Fazilabs Home" className="flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-[color:var(--color-gold)] min-w-[40px]">
            <img
              ...
              alt="Fazilabs logo"
              className="w-[96px] md:w-[100px] h-auto min-w-[40px] max-w-[180px] object-contain drop-shadow-sm"
              loading="eager"
              draggable="false"
              style={{ display: 'block' }}
            />
      ...
      {/* Mobile Menu Overlay */}
      {menuOpen && (
        <div className="fixed inset-0 z-[60] bg-gradient-to-br from-[color:var(--color-primary)] to-[color:var(--color-secondary)] backdrop-blur-md text-white flex flex-col items-center justify-center transition-all">
          <div className="absolute inset-0 bg-gradient-to-br from-[color:var(--color-primary)] to-[color:var(--color-secondary)] opacity-95" />
          <div className="relative z-10 w-full max-w-[90vw] mx-auto px-4 md:px-8">
            <button
              className="absolute top-0 right-0 p-2 rounded hover:bg-[color:var(--color-gold)]/10"
            ...
            <ul className="flex flex-col gap-8 text-2xl font-semibold mt-16">
              {links.map((link) => (
                <li key={link.name}>
                  {link.href.startsWith('/') ? (
                    <Link
      ...
      <button
        onClick={scrollToTop}
        className={`fixed bottom-8 right-8 z-40 p-3 rounded-full bg-gradient-to-r from-[color:var(--color-primary)] to-[color:var(--color-secondary)] text-white shadow-lg transition-all duration-300 hover:scale-110 ${
